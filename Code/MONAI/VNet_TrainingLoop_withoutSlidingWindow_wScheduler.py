...
def VNET_TRAINING_withoutSlidingWindow_withScheduler(model,
             NUM_CLASSES: int,
             MAX_EPOCHS: int,
             VAL_INTERVAL: int,
             train_loader,
            ...
            with torch.no_grad():
               # for val_data in tqdm(val_loader):
                ...
                def HelperBestMetric(name, metric, best_metric, best_metric_epoch, indicator):
                    ...
                    file = os.path.join(root_dir, 'Model', 'Save', f"VNET_best_metric_{name}_model_{str(epoch)}_{formatted_date}_{indicator}.pth")
                ...
                for name, metric in metric_results.items():  # ToDo: choose what is best depending on metric (dice higher, hausdorff lower). implement this dependence. at the moment hardcoded
                    ...
                    if 'hausdorff' in name.lower():
                        if metric < best_metric[name] and not math.isinf(metric):
